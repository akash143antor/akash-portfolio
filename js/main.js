...
(() => {
    ...
    document.addEventListener("click", (event) => {
        if (event.target.classList.contains("link-item") && event.target.hash !== "") {
            ...
            // Update active navigation link in nav menu
            ...
            updateSidebarNav(hash);

            // Update URL hash
            window.location.hash = hash;

            // Re-init AOS for the newly visible section
    ...
    // Handle sidebar nav clicks specifically
    document.querySelectorAll('.sidebar-nav-item').forEach(item => {
        item.addEventListener('click', (e) => {
            // The link-item class handler above will handle the logic
            // but we need to ensure sidebar items also trigger section switching
        });
    });
})();
...
(() => {
    ...
    tabsContainer.addEventListener("click", (event) => {
        if (event.target.classList.contains("tab-item") && !event.target.classList.contains("active")) {
            ...
            // Re-init AOS for the newly visible content
            if (typeof AOS !== 'undefined') {
                setTimeout(() => AOS.refresh(), 100);
            }
        }
